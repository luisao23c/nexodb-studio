...
class DynamicTableService
{
    ...
    public function modifyColumn(string $table, string $column, array $physical): void
    {
        ...
        if (! empty($physical['unique'])) {
            ...
            if (! $exists) {
                DB::statement("ALTER TABLE `{$table}` ADD UNIQUE KEY `{$indexName}` (`{$column}`)");
            }
        }
    }
    ...
    public function addColumnRaw(string $table, string $name, array $data): void
    {
        ...
        if (! empty($data['unique'])) {
            DB::statement("ALTER TABLE `{$table}` ADD UNIQUE KEY `{$name}_unique` (`{$name}`)");
        }
    }
    ...
    public function columnDefinition(array $physical): string
    {
        ...
        if ($template === null) {
            throw ValidationException::withMessages(['data_type' => 'Tipo de dato no permitido.']);
        }

        $result = str_replace(':length', (string) (($physical['length'] ?? null) ?: 255), $template);
        if ($type === 'enum' && ! empty($physical['enum_values'])) {
            $values = array_map(fn ($v) => "'".str_replace("'", "''", trim($v))."'", explode(',', $physical['enum_values']));
