...
export function RecordsView({module}:{module:Module}) {
  ...
  function display(row:Row,field:Field){const value=field.data_type==='relation'?row[`${field.name}_display`]??row[field.name]:row[field.name];if(field.data_type==='boolean')return value?<span className="yes">Sí</span>:<span className="no">No</span>;return value===null||value===''||value===undefined?<span className="muted">—</span>:String(value)}
